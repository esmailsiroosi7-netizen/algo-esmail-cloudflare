...
// Calibrated-style confidence: the score is no longer a raw dominance
ratio.

// It rewards directional edge, while reserving room for quality
filters.
...
const direction = edge \>= 10 ? (bull \> bear ? \"خرید\" : \"فروش\") :
\"خنثی\";

const total = bull + bear;

const score = Math.round(clamp(50 + (total ? edge / total \* 35 : 0) +
(direction === \"خنثی\" ? 0 : 15), 0, 95));
...
// Execution confirmation: the lower timeframe must agree with the
signal.
...
if (combined.direction === \"خرید\" && lowerBull \<
MIN_LOWER_TF_ALIGNMENT) {

combined.direction = \"خنثی\";

}

if (combined.direction === \"فروش\" && lowerBear \<
MIN_LOWER_TF_ALIGNMENT) {
...
// Use the latest ticker-derived price when available, not an older
candle close.
...
const positionNotional = stopPercent \> 0 ? riskAmount / stopPercent :
PAPER_BUDGET;
...
grossPnl = (priceMovePercent / 100) \*
safeNumber(trade.positionNotional);
...
fundingPnl = signed \* rate \* safeNumber(trade.positionNotional) \*
fundingPeriods;
...
📈 حرکت قیمت: \${safeNumber(trade.priceMovePercent ??
trade.pnlPercent).toFixed(2)}%

📊 بازده روی مارجین:
\${safeNumber(trade.marginReturnPercent).toFixed(2)}%
...
const funding = item.funding == null ? \"نامشخص\" :
item.funding.toFixed(6);

===== Page 102 =====

const ls = item.longShort == null ? \"نامشخص\" :
...
📌 حرکت قیمت سربه‌سر از بابت کارمزد:
\${trade.feeBreakEvenPriceMovePercent.toFixed(3)}%
...
⚠️ این بخش فقط برای تشخیص مشکل است و هنوز هیچ تغییری در استراتژی ایجاد
نمی‌کند.
...
const closed = trades.filter(t =\> t.status === \"WIN\" \|\| t.status
=== \"LOSS\");
...
const net = closed.reduce((a,t) =\> a + safeNumber(t.pnlUsdt ?? t.pnl),
0);
...
const funding = closed.reduce((a,t) =\> a + safeNumber(t.fundingPnl),
0);

const avgMarginReturn = average(closed.map(t =\>
safeNumber(t.marginReturnPercent)));

===== Page 137 =====

const avgPriceMove = average(closed.map(t =\>
safeNumber(t.priceMovePercent ?? t.pnlPercent)));

const winRate = closed.length ? wins.length / closed.length \* 100 : 0;

const profitFactor = Math.abs(losses.reduce((a,t) =\> a + Math.min(0,
safeNumber(t.pnlUsdt ?? t.pnl)), 0)) \> 0

? wins.reduce((a,t) =\> a + Math.max(0, safeNumber(t.pnlUsdt ?? t.pnl)),
0) /

Math.abs(losses.reduce((a,t) =\> a + Math.min(0, safeNumber(t.pnlUsdt ??
t.pnl)), 0))
