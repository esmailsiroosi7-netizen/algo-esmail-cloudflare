// ============================================================
// ALGO FJM V6.1 REGIME SCANFIX - Toobit Futures Analyzer
...
// ALGO FJM V6.1 REGIME - Futures-first configuration
...
async function getStats(
  ...
  return `
📊 *آمار معاملات آزمایشی ALGO FJM V6.1 REGIME*
...
`;
  ...
  let text = `
📚 *تاریخچه معاملات ALGO FJM V6.1 REGIME*
...
async function sendMenu(chatId, env, text = "🤖 *ALGO FJM V6.1 REGIME*\n\nیک گزینه را انتخاب کن:") {
...
نتیجه پس از پایان اسکن ارسال می‌شود.`,
      ...
      // /scan از fetch با ctx.waitUntil اجرا می‌شود.
      // ابتدا خود بازار را اسکن می‌کنیم تا بررسی معاملات باز مانع گزارش بازار نشود.
      try {
        console.log(
          "SCAN START",
          chatId
        ...
        // 3) معاملات قبلی بعد از اسکن بررسی می‌شوند
        let tradeUpdate = {
          checked: 0,
          closed: 0,
          ambiguous: 0,
        ...
        // 4) گزارش بازار حتماً مستقل از خطای معاملات قبلی ارسال شود
        const report =
          buildScanReport(
            scan.results,
            scan.btcContext,
  ...
  async fetch(
    ...
    if (
      ...
      try {
        ...
        // /scan را مستقیم در یک waitUntil مستقل اجرا کن.
        // این مسیر از پردازش عمومی جداست تا هیچ شاخه دیگری اسکن را متوقف نکند.
        if (
          command === "/scan" &&
          update?.message?.chat?.id
        ) {
